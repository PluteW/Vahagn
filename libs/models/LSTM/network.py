...
class Basic_network(nn.Module):
    def __init__(self, base_network='vgg_16', pretrained=False,out_dim=64):
        ...
        if base_network == 'vgg_16':
            ...
            self.fc = nn.Sequential(nn.Linear(4096*2, out_dim))
            ...
            self.fc = nn.Sequential(nn.Linear(2048*2, out_dim))
...
class Slip_detection_network(nn.Module):
    def __init__(self, base_network='vgg_16', pretrained=False, 
                 frame=8,
                 rnn_input_size=64, rnn_hidden_size=64,
                 rnn_num_layers=1, num_classes=2, use_gpu=False, dropout=0.8):
        super(Slip_detection_network, self).__init__()
        self.cnn_network = Basic_network(base_network=base_network, pretrained=pretrained, out_dim=rnn_input_size)
    ...
    def forward(self, x_1, x_2):
        ...
        """
        for i in range(2,self.frame):
            if self.use_gpu:
                features = self.cnn_network(((128+x_1[i]-x_1[0])/255).to(device),((128+x_2[i]-x_2[0])/255).to(device))
            else:
                features = self.cnn_network((128+x_1[i]-x_1[0])/255, (128+x_2[i]-x_2[0])/255)
